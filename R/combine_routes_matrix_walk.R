...
ttmat_text = "ttmatrix_walk_chunk_"
routes_text = "routes_walk_"
out_text = "data/ttmatrix/final/walk.csv"
threshold = 1
with_routes = TRUE
...
ntem <- st_read("data/NTEM/NTEM_centroids_mod.geojson")
ntem <- ntem[match(rownames(newmat4), ntem$Zone_Code),]
ntem$times <- newmat4[,"S99900207"]
qtm(ntem, dots.col = "times") + qtm(ntem[ntem$Zone_Code == "S99900207",], dots.col = "red")
...
mat_t <- t(newmat4)
newmat6 <- ifelse(is.na(newmat4), mat_t, newmat4)

newmat7 <- odmatrix_interpolate(newmat6, 1) # Final Pass

ntem <- st_read("data/NTEM/NTEM_centroids_mod.geojson")
ntem <- ntem[match(rownames(newmat7), ntem$Zone_Code),]
csum <- rowSums(is.na(newmat7))
ntem$csum <- csum
ntem <- ntem[ntem$csum > min(ntem$csum, na.rm = TRUE),]
qtm(ntem, dots.col = "csum")
...
ntem <- st_read("data/NTEM/NTEM_centroids_mod.geojson")
ntem <- ntem[match(rownames(newmat7), ntem$Zone_Code),]
ntem$times <- newmat7[,"S99900498"]
qtm(ntem, dots.col = "times") + qtm(ntem[ntem$Zone_Code == "S99900498",], dots.col = "red")
