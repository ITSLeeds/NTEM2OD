...
for(i in 9:length(chunks)){
  ...
  saveRDS(routes, paste0("data/ttmatrix/routes_walk_chunk_",i,"_from_",min(chunk_sub),"_to_",max(chunk_sub),".Rds"))
...
fromPlace <- read_sf("data/NTEM/NTEM_centroids_walk_from.geojson")
toPlace <- read_sf("data/NTEM/NTEM_centroids_walk_to.geojson")

fromPlace <- rbind(fromPlace, toPlace)
toPlace <- rbind(toPlace, fromPlace[seq(1, nrow(toPlace)),])
...
saveRDS(routes3, paste0("data/ttmatrix/routes_walk_bonus.Rds"))
...
# Read in routes
files <- list.files("data/ttmatrix/", pattern = "routes_walk_chunk_", full.names = TRUE)
