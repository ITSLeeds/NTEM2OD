...
curves <- readRDS("data/NTS/decay_curves.Rds")
...
for(i in 1:nrow(mat_types)){
  ...
  if(purpose_sub == "Business" ){
    pup <- "business"
  } else if (purpose_sub == "Commuting") {
    pup <- "work"
  } else if (purpose_sub == "Education / escort education") {
    pup <- "education"
  } else if (purpose_sub == "Leisure") {
    pup <- "holiday"
  } else if (purpose_sub == "Personal business") {
    pup <- "personal"
  } else if (purpose_sub == "Shopping") {
    pup <- "shopping"
  } else {
    stop("Unknown purpose")
  }
  ...
  origins <- ntem_sub[,paste0("HB_",pup,"_origin")]
  destinations <- ntem_sub[,paste0("HB_",pup,"_destination")]
  ...
  origins <- ntem_sub[,paste0("NHB_",pup,"_origin")]
  destinations <- ntem_sub[,paste0("NHB_",pup,"_destination")]
  ...
  # Make the non-Home Based Matrix 
  
  mat_nhb <- matrix(destinations, ncol = 1) %*% matrix(origins, ncol = 7700)
  mat_nhb <- mat_nhb * mat_sub
  mat_nhb <- round(mat_nhb, 10)
  mat_nhb[is.infinite(mat_nhb)] <- 0
  
  mat_nhb = furness(mat_nhb, destinations, origins, 20)
  
  # Check totals
  total_mat = sum(mat_nhb, na.rm = TRUE)
  total_orig = sum(origins)
  total_dest = sum(destinations)
  ...
  mat_nhb <- od_to_flow(mat_nhb)
  
  mat_hb$flow <- round(mat_hb$flow)
  mat_nhb$flow <- round(mat_nhb$flow)
  
  names(mat_hb) <- c("from", "to",paste0("HB_",md,"_",pup))
  names(mat_nhb) <- c("from", "to",paste0("NHB_",md,"_",pup))
  
  list_flow[[i]] <- list(mat_hb, mat_nhb)
...
flow_all = list_flow[[1]][[1]]
flow_all <- flow_all[!is.na(flow_all$HB_cycle_business),]
flow_all = flow_all[flow_all$HB_cycle_business > 0,]
flow_nhb <- list_flow[[1]][[2]]
flow_nhb = flow_nhb[!is.na(flow_nhb$NHB_cycle_business),]
flow_nhb = flow_nhb[flow_nhb$NHB_cycle_business > 0,]
flow_all = full_join(flow_all, flow_nhb, by = c("from","to"))


for(i in 2:length(list_flow)){
  message(Sys.time()," ",i," nflows = ",nrow(flow_all))
  flow_hb <- list_flow[[i]][[1]]
  flow_nhb <- list_flow[[i]][[2]]
  flow_hb <- flow_hb[!is.na(flow_hb[,3]),]
  flow_nhb <- flow_nhb[!is.na(flow_nhb[,3]),]
  flow_hb <- flow_hb[flow_hb[,3] > 0,]
  flow_nhb <- flow_nhb[flow_nhb[,3] > 0,]
  message(nrow(flow_hb)," and ",nrow(flow_nhb))
...
tm_shape(foo) +
  tm_lines(lwd = "drive", 
           ...
           style = "fixed",
           scale = 5,
           breaks = c(0,100,200,300,400,500,600,800,1000,15000,40000))
